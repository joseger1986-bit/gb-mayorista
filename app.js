...
const CATALOG_HIGH_PRIORITY_IMAGE_COUNT = 2;
...
function setupSupabaseOrdersRealtime() {
  const client = getSupabaseCatalogClient();
  if (!client || typeof client.channel !== "function") return false;
  if (!isPrivateManagementRoute() || !internalUnlocked || !hasPermission("orders")) return false;
  if (supabaseOrdersRealtimeChannel) return true;
  ...
  supabaseOrdersRealtimeChannel = client
    .channel("punto-x-mayor-gestion-orders")
    .on("postgres_changes", { event: "INSERT", schema: "public", table: "orders" }, refreshOrdersFromEvent)
    .on("postgres_changes", { event: "UPDATE", schema: "public", table: "orders" }, refreshOrdersFromEvent)
    .on("postgres_changes", { event: "*", schema: "public", table: "order_items" }, refreshOrdersFromEvent)
    .subscribe((status) => {
      const previousStatus = supabaseOrdersRealtimeStatus;
      supabaseOrdersRealtimeStatus = String(status || "").toUpperCase();
      document.documentElement.dataset.gbOrdersRealtime = supabaseOrdersRealtimeStatus.toLowerCase();
      logSupabaseSyncDebug(status === "SUBSCRIBED" ? "orders-realtime-ok" : "orders-realtime-status", {
        message: `Estado del canal Realtime de pedidos: ${status}`,
        status
      }, status === "CHANNEL_ERROR" || status === "TIMED_OUT" || status === "CLOSED" ? "error" : "info");
      if (status === "SUBSCRIBED" && previousStatus && previousStatus !== "SUBSCRIBED") {
        refreshOrdersFromSupabase("orders-realtime-reconnect", { silent: true });
      }
    });
...
function teardownSupabaseOrdersRealtime() {
  window.clearTimeout(supabaseOrdersRealtimeRefreshTimer);
  supabaseOrdersRealtimeRefreshTimer = null;
  supabaseOrdersRealtimeStatus = "";
  const client = getSupabaseCatalogClient();
  const channel = supabaseOrdersRealtimeChannel;
  supabaseOrdersRealtimeChannel = null;
  ...
  els.productGrid.innerHTML = filtered.map((group, index) => `
    <article class="product-card ${hasCatalogVariantChoices(group) ? "has-variants" : "no-variants"}">
      <button class="product-image-button" type="button" data-open-gallery="${escapeHtml(group.id)}" aria-label="Ver fotos de ${escapeHtml(group.name)}">
        <img class="product-image" src="${escapeHtml(group.image)}" alt="${escapeHtml(group.name)}" loading="${index < priorityImageCount ? "eager" : "lazy"}" decoding="async" fetchpriority="${index < CATALOG_HIGH_PRIORITY_IMAGE_COUNT ? "high" : "low"}" data-catalog-image="${escapeHtml(group.id)}" data-image-index="0" onerror="showNextCatalogImage(this)">
      </button>
...
function preloadCatalogImages(groups, priorityImageCount = getCatalogPriorityImageCount()) {
  ...
  const urls = groups
    .slice(priorityImageCount, priorityImageCount + 3)
    .map((group) => group.image)
    .filter((url) => url && url !== DEFAULT_PRODUCT_IMAGE && !String(url).startsWith("data:"))
    .filter((url, index, list) => list.indexOf(url) === index);
  if (!urls.length) return;
  catalogImagePreloadTimer = window.setTimeout(() => {
    const run = () => warmCatalogImageQueue(urls, 1);
...
function moveEditProductPhoto(fromIndex, toIndex) {
  if (fromIndex < 0 || fromIndex >= editProductPhotoItems.length) return;
  const nextIndex = Math.max(0, Math.min(Number(toIndex) || 0, editProductPhotoItems.length - 1));
  const [item] = editProductPhotoItems.splice(fromIndex, 1);
  editProductPhotoItems.splice(nextIndex, 0, item);
...
async function handleProductImagePreview() {
  ...
  try {
    ...
    if (els.imagePreviewText) els.imagePreviewText.textContent = `${previews.length} foto(s) seleccionada(s). La primera sera la principal.`;
...
function resetImagePreview(message = "La vista previa de la foto original aparecera antes de guardar.") {
  ...
  if (els.imagePreviewText) els.imagePreviewText.textContent = message || "La vista previa de la foto original aparecera antes de guardar.";
...
function renderEditProductImageGallery(product) {
  ...
  if (!editProductPhotoItems.length && product && !editProductRemoveImagePending) editProductPhotoItems = getStoredProductImages(product).map((src) => ({ type: "existing", src }));
  const images = editProductPhotoItems;
  const cover = images[0]?.src || DEFAULT_PRODUCT_IMAGE;
...
function getEditProductFormState() {
  ...
  return JSON.stringify({
    ...
    photoDraft: editProductPhotoItems.map((item) => ({ type: item.type, src: item.src, name: item.file?.name || "" }))
  });
}
...
async function replaceProductGalleryImage(index, file) {
  await replaceEditProductPhotoItem(index, file);
}
...
function isPrivateManagementRoute() {
  if (isManagementHostname()) return true;
  if (isPublicCatalogHostname()) return false;
  return normalizeRoutePath(window.location.pathname) === PRIVATE_MANAGEMENT_PATH;
}

function isManagementHostname() {
  const hostname = String(window.location.hostname || "").toLowerCase();
  return MANAGEMENT_HOSTNAMES.includes(hostname);
}

function isPublicCatalogHostname() {
  const hostname = String(window.location.hostname || "").toLowerCase();
  return PUBLIC_CATALOG_HOSTNAMES.includes(hostname);
}
