...
function renderOrdersToolbar() {
  const filters = ["Todas", "En revisi�n", "Pagadas"];
  return `
...
function normalizeConsultationStatus(status) {
  return ["Pagado", "Entregado", "Preparado", "Preparados"].includes(status) ? "Pagado" : "En revisi�n";
}
function matchesOrderListFilter(order, filter) {
  if (filter === "En revisi�n") return normalizeConsultationStatus(order.status) === "En revisi�n";
...
function renderOrderCustomerBlock(order) {
  ...
  if (!isEditing) {
    return `
      <div class="compact-customer-readonly">
        ...
        <span><b>Tel�fono</b>${escapeHtml(order.customerPhone || "Sin tel�fono")}</span>
  ...
  return `
    <div class="customer-edit-grid compact-customer-grid">
      ...
      <label>Tel�fono<input type="tel" value="${escapeHtml(order.customerPhone || "")}" data-budget-customer-phone="${order.id}" ${canEditOrder(order) ? "" : "disabled"}></label>
          ...
          <div class="checkout-grid compact-checkout-grid">
            ...
            ${getNormalizedDeliveryType(order.deliveryType) === "Transporte" ? `
              <label class="conditional-delivery-field">
                Transporte
                <input type="text" value="${escapeHtml(order.transport || "")}" placeholder="Kelly, V�a Cargo, Expreso..." data-budget-transport="${order.id}" ${canEditOrder(order) ? "" : "disabled"}>
              </label>
            ` : ""}
    ...
    <div class="budget-item-edit-modal" role="dialog" aria-modal="true" aria-label="Editar producto del pedido" data-budget-item-edit-overlay>
      <section class="budget-item-edit-dialog">
        <div class="budget-preview-head">
          ...
          <button class="icon-button" type="button" data-close-budget-item-edit aria-label="Cerrar edici�n">�</button>
        </div>
        <div class="budget-item-edit-grid">
          <label>Presentaci�n<input type="text" value="${escapeHtml(presentation)}" data-edit-item-presentation></label>
...
function saveBudgetItemEdit(orderId, productId) {
  ...
  if (!presentation) {
    showToast('La presentaci�n no puede quedar vac�a');
    return;
  }
...
function saveManualConsultation(order) {
  ...
  if (!customerName) {
    ...
    renderOrders();
    showToast("Complet� el nombre del cliente");
    return false;
  }
...
function sendBudgetPreviewByWhatsapp(orderId) {
  ...
  if (normalizeConsultationStatus(order.status) !== "Pagado") order.status = "En revisi�n";
  order.updatedAt = new Date().toISOString();
  recalculateBudget(order);
  previewOrderId = "";
  saveOrders();
...
function updateRecordStatus(id, status) {
  ...
  if (status === "En revisi�n" && order.stockApplied) {
    showToast("La consulta ya est� pagada y el stock descontado");
    renderOrders();
    return;
  }
  order.status = status;
